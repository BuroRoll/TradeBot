...
class SuperTrendStatus(NamedTuple):
    """
    Текущее состояние супертренда
    """
    status: str


def get_status() -> SuperTrendStatus:
    symbol = 'ETH-USD'
    df = yf.download(symbol, start='2023-01-18', interval="1h", progress=False)
    df_super_trend = SuperTrend(df, 10, 3)
    status = SuperTrendStatus(status=df_super_trend['STX_10_3'].iloc[-1])
    return status
